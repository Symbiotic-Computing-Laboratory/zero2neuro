...
- __Dropout__: 
   - ```--dropout`` (float): during training, randomly deactivate neurons within the layer with the specified probability (default = none).  This implements an implicit form of regularization.  Reasonable values for this argument are 0.1 ... 0.5
...
Components:
- __Inputs__: 
   - ```--input_shape``` specifies the shape of the input to the neural network.  Typically, this is a single integer. 
...
- __Embedding + Flatten__: translates a sequence of integers into a sequence of vectors that are then appended together to form one large vector
