...
def build_location_grid(loc_df, feature_cols, years, epiweeks, data_outputs):
    ...
    for yi, year in enumerate(years):
        for ei, epiweek in enumerate(epiweeks):
            ...
            if (year, epiweek) not in indexed.index:
                # Forward-fill: If not the first week, copy previous week's values
                if ei > 0:
                    for col in feature_cols:
                        grids[col][yi, ei, 0] = grids[col][yi, ei - 1, 0]
                
                # Note: data_mask remains 0.0 so the network knows it's imputed
                continue
        ...
        # -------------------------------------------------------------------
        # Backward-fill the start of the year if the first weeks were missing
        # -------------------------------------------------------------------
        first_valid_ei = -1
        for ei in range(n_epiweeks):
            if grids['data_mask'][yi, ei, 0] == 1.0:
                first_valid_ei = ei
                break
        
        if first_valid_ei > 0:
            for ei in range(first_valid_ei):
                for col in feature_cols:
                    grids[col][yi, ei, 0] = grids[col][yi, first_valid_ei, 0]
    ...
    # -----------------------------------------------------------------------
    # CRITICAL FIX: Ensure the target variables (data_outputs) are zeroed out 
    # anywhere data_mask is 0.0. This prevents plot_true_vs_pred.py from 
    # plotting or summing up the fake forward-filled mosquito counts!
    # -----------------------------------------------------------------------
    for out_col in data_outputs:
        grids[out_col] *= grids['data_mask']
