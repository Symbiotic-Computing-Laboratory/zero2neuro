...
def main():
    ...
    # 1. Figure out the original Test Set (Fold 4) using the default seed
    rng_default = np.random.default_rng(args.data_seed)
    shuffled_default = all_locations.copy()
    rng_default.shuffle(shuffled_default)
    
    # zero2neuro default test fold is typically fold_4 if N_FOLDS=5
    test_fold_idx = args.data_n_folds - 1
    original_loc_to_fold = {loc: i % args.data_n_folds for i, loc in enumerate(shuffled_default)}
    original_test_locs = [loc for loc, fidx in original_loc_to_fold.items() if fidx == test_fold_idx]
    
    # 2. Define the hardcoded Train and Val sets
    custom_train = ['R117-C385', 'R118-C380', 'R132-C372', 'R132-C381', 'R135-C383']
    # custom_train = ['R117-C385', 'R118-C380', 'R124-C379', 'R132-C381', 'R137-C378']
    custom_val = ['R140-C378']
    
    # Make sure we don't accidentally put a custom train/val station in the test set
    custom_test = [loc for loc in original_test_locs if loc not in custom_train and loc not in custom_val]

    print(f'\n[2] Applying Manual Location Filters:')
    print(f'    Training Stations   : {len(custom_train)}')
    print(f'    Validation Stations : {len(custom_val)}')
    print(f'    Testing Stations    : {len(custom_test)} (Derived from random seed {args.data_seed})')

    # Combine them to define our new universe of locations
    active_locations = sorted(custom_train + custom_val + custom_test)
    ...
    for i, loc in enumerate(custom_train):
        loc_to_fold[loc] = i % 3  # distribute over fold 0, 1, 2
        
    for loc in custom_val:
        loc_to_fold[loc] = 3      # validation is fold 3
        
    for loc in custom_test:
        loc_to_fold[loc] = 4      # test is fold 4
    ...
    for fold_idx in range(args.data_n_folds):
        ...
        if n_locs_in_fold == 0:
            ...
            # Save empty arrays so the zero2neuro loader doesn't crash reading a missing file
            empty_dict = {key: np.zeros((0, n_epiweeks, 1), dtype=np.float32) for key in all_pkl_keys}
