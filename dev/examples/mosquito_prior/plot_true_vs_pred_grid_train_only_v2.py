...
def main():
    ...
    mask_idx = res['args'].data_inputs.index('data_mask')
    ...
    for y in range(N_YEARS):
        ...
        all_mask = np.zeros(len(EPIWEEKS))
        ...
        for c_idx, loc in enumerate(ordered_locs):
            ...
            mask = ins[loc_row_idx, :, mask_idx]
            
            # True values are 0 during padded weeks anyway, so direct sum is fine
            all_true += y_t
            # Add all predictions (even during padded weeks) because we want to see the full prediction line
            all_pred += y_p
            all_mask = np.logical_or(all_mask, mask > 0)
            
            ax = axes[y, c_idx + 1]
            is_bottom = (y == N_YEARS - 1)
            plot_on_axis(ax, y_t, y_p, mask, is_bottom)
        ...
        # Apply the global mask so we don't plot weeks where every single location is padded
        plot_on_axis(ax_all, all_true, all_pred, all_mask, is_bottom_all)
...
def plot_on_axis(ax, y_t, y_p, mask, is_bottom_row):
    ax.set_facecolor(PANEL)
    
    if mask is not None:
        valid_idx = np.where(mask > 0)[0]
    else:
        valid_idx = np.arange(len(y_t))
        
    # ALWAYS plot prediction across all epiweeks!
    ax.plot(EPIWEEKS, y_p, color=PRED_C, linewidth=2.0, linestyle='--', label='Pred')
    
    if len(valid_idx) > 0:
        valid_epi = np.array(EPIWEEKS)[valid_idx]
        
        # Plot TRUE value only on valid epiweeks, skipping padded zeros!
        ax.plot(valid_epi, y_t[valid_idx], color=TRUE_C, linewidth=2.0, label='True')
        
        # Interpolate the true value so we can shade between True and Pred cleanly across gaps
        y_t_interp = np.interp(EPIWEEKS, valid_epi, y_t[valid_idx])
        ax.fill_between(EPIWEEKS, y_t_interp, y_p, alpha=0.15, color='white')
