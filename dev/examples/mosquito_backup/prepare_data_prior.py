...
def build_location_grid(loc_df, feature_cols, years, epiweeks, data_outputs):
    ...
    # by flattening the time series across years, rolling by 1, and reshaping.
    for out_col in data_outputs:
        prior_col = f"{out_col}.prior"
        original = grids[out_col]
        
        flat_original = original.reshape(-1)
        flat_prior = np.roll(flat_original, 1)
        # Handle the very first timestep which has no prior
        if len(flat_prior) > 0:
            flat_prior[0] = 0.0
            
        grids[prior_col] = flat_prior.reshape(n_years, n_epiweeks, 1)
